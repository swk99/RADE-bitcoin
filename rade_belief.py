...
class KDEThreshold:
    ...
    def update(self, value: float):
        """Add a new observation to the rolling KDE window."""
        self._samples.append(value)

    def _kde_pdf(self, x: float) -> float:
        """Gaussian KDE density at x."""
        n = len(self._samples)
        if n == 0:
            return 0.0
        h = self.bandwidth
    ...
    def _kde_cdf(self, x: float) -> float:
        """Approximate CDF at x via trapezoidal integration on a grid."""
        if not self._samples:
            return 0.0
        lo  = min(self._samples) - 3 * self.bandwidth
        hi  = max(self._samples) + 3 * self.bandwidth
        xs  = [lo + (hi - lo) * i / self.n_grid for i in range(self.n_grid + 1)]
        ys  = [self._kde_pdf(xi) for xi in xs]
        cdf = 0.0
        for i in range(self.n_grid):
            dx   = xs[i + 1] - xs[i]
            cdf += 0.5 * (ys[i] + ys[i + 1]) * dx
            if xs[i + 1] >= x:
                break
        return min(cdf, 1.0)
    ...
    def threshold(self, belief: float) -> float:
        ...
        for _ in range(40):
            mid = (lo + hi) / 2.0
            if self._kde_cdf(mid) < target_cdf:
                lo = mid
            else:
                hi = mid
